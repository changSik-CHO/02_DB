...
-- EMPLOYEE 테이블에서 사번, 직원이름, 휴대전화번호 컬러만 조회
...
-- SALATY 자체는 안에 숫자타입이여서 가능한거고, 

SELECT '같음'
FROM DUAL 
WHERE 1 = '1';
-- DUAL(DUmmy tAbLe) : 가짜 테이블(임시 조회용 테이블)
-- 더미테이블에서 같음이라고 조회할건데 숫지1이랑 문자1이 같다고 인식
...
-- NOT 연산자 사용가능 BETWEEN 앞에 작성, NOT 자체가 부정형 같음
SELECT EMP_ID, EMP_NAME, SALARY, PHONE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블 에서 전화번호가 010으로 시작하지 않는 사원
-- 사번, 이름, 전화번호 조회
...
-- Hint! AND 가 OR 보다 우선순위가 높다, () 사용 가능
...
FROM EMPLOYEE 
WHERE DEPT_CODE IN('D1','D6','D9'); -- 12명
