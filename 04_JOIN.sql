...
-- 성이 '전'씨인 직원들의 사원명, 주민번호, 부서명, 직급명을 조회하시오.
SELECT EMP_NAME, EMP_NO, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE EMP_NO LIKE '7%' -- 70년대 생
AND SUBSTR(EMP_NO, 8, 1) = '2' -- 여자
AND EMP_NAME LIKE '전%';---- 성이 전씨
...
SELECT EMP_NAME 사원명, DEPT_CODE 부서코드, EMP_NAME 동료이름
FROM EMPLOYEE E1
