...
-- 2) 부서별 급여합이 17,700,000인 부서의 부서명과 급여합 조회(메인)
...
-- 2. 2000년에 입사한 사원의 부서와 직급이 같은 사원을 조회
...
-- 2) 메인쿼리 조회 시 인라인뷰를 이용하여 급여 상위 5명 까지 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY
	FROM EMPLOYEE
	ORDER BY SALARY DESC) -- 해석순위1위 FROM 절에서 전체 직원의 SALARY 내림차순 정렬
WHERE ROWNUM <= 5; -- 해석순위2위 WHERE절에서 이제 가상컬럼의 1~5행까지만 조회
