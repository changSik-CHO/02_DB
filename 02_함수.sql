...
SELECT EMP_NO, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1) 아이디
FROM EMPLOYEE;
...
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-10-20 09:15:57.947 +0900
...
SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 2025-09-20 09:30:38.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL; -- 2025-10-31 09:32:08.000
...
SELECT HIRE_DATE FROM EMPLOYEE; -- 2008-04-02 00:00:00.000
...
-- TO_CHAR(날짜) 이용하여 문자형으로 변경 후 TO_NUMBER(문자) 숫자로 변경
...
-- DECODE(계산식 | 컬럼명, 
-- 조건값1, 선택값1, 조건값2, 선택값2 ..., 아무것도 일치하지 않을때)
...
-- 급여 최대값, 가장 늦은 입사일, 알파벳순서가 가장 늦은 이메일 조회
