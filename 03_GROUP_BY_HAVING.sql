...
-- 여러 개의 결과값을 산출하기 위해 그룹함수가 적용된 
-- 그룹의 기준을 ORDER BY절에 기술하여 사용
...
-- 직급코드가 같은 사람의	 (GROUP BY 절 : 직급을 그룹으로 묶기 JOB_CODE)
-- 직급코드, 급여 평균, 인원 수를 (SELECT 절)
-- 직급코드 오름차순으로 조회 (ORDER BY절)
...
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 X (SELECT절 사용 안됨)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 절 해석 완료된 후)
...
-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서코드가 D5, D6인 부서의 (WHERE -> GROUP BY절)
...
GROUP BY DEPT_CODE;
-- 부서의 (WHERE절에서 구한 부서코드가 D5,D6인 데이터들을 그룹으로 묶음)

-- EMPLOYEE 테이블에서 (FROM절)
-- 2000년도 이후 입사들의 (WHERE 절 -> GROUP BY절)
-- 직급별 급여합을 조회 
-- 직급코드, 급여합 조회(SELECT 절)
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 -- 한 사람의 급여가 3백만원 이상이라는 조건
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 -- DEPT_CODE 그룹 중 급여 평균이 3백만원 이상인 그룹만 조회(요구사항O)
ORDER BY DEPT_CODE;
